...
function true_id_doctor2($args){
	$args['Familiya2'] = 'Фамилия';	
	$args['dolzhnost2'] = 'Должность';		
	$args['opyt_raboty2'] = 'Опыт работы';
	return $args;
}
function true_custom_doctor2($column, $id){
	if($column === 'Familiya2'){
		if (get_post_meta($id, 'Familiya2', true)) {echo get_post_meta($id, 'Familiya2', true);}
	}
	if($column === 'dolzhnost2'){
		if (get_post_meta($id, 'dolzhnost2', true)) {echo get_post_meta($id, 'dolzhnost2', true);}
	}	
	if($column === 'opyt_raboty2'){
		if (get_post_meta($id, 'opyt_raboty2', true)) {echo get_post_meta($id, 'opyt_raboty2', true);}
	}
}
